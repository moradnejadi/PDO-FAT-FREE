...
class Multilang extends Prefab
{
    ...
    function displayLanguage($iso)
    {
        ...
        return class_exists('Locale') ? \Locale::getDisplayLanguage($iso,
            $this->locale()) : constant('ISO::LC_' . $iso);
    }
    ...
    function displayCountry($iso)
    {
        ...
        return class_exists('Locale') ? \Locale::getDisplayRegion('-' . $iso,
            $this->locale()) : constant('ISO::CC_' . $iso);
    }
    ...
    protected function detect($uri = null)
    {
        ...
        if (preg_match('/^' . preg_quote($this->f3->get('BASE'), '/') . '\/([^\/?]+)([\/?]|$)/',
                $uri ?: $_SERVER['REQUEST_URI'], $m) &&
            array_key_exists($m[1], $this->languages)) {
            $this->current = $m[1];
        } else {//auto-detect language
            $this->auto = true;
            $detected = array_intersect(explode(',', $this->f3->get('LANGUAGE')),
                explode(',', implode(',', $this->languages)));
